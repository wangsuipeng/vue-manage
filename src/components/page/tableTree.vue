...
export default { 
	name: 'tree-grid', 
	props: { treeStructure: { type: Boolean, default: true }, 
	columns: { type: Array, default: () => [] }, 
	dataSource: { type: Array, default: () => [] }, 
	treeType: { type: String, default: 'normal' }, 
	defaultExpandAll: { type: Boolean, default: true }, 
	fixedHead: { type: String, default: '' }, 
	fixed: { type: Boolean, default: true }, 
	btnList: { type: Array, default: () => ([]) }, 
	operatWidth: { type: Number, default: 300 } },
	 data() { return {} }, 
	 computed: { 
		 data() { 
	...
	methods: { 
		...
		showTr(rows, index) { 
			...
			toggle(trIndex) { 
				...
				spaceIconShow(index) { if (this.treeStructure && index === 0) { return true } return false }, toggleIconShow(index, record) { if (this.treeStructure && index === 0 && record.children && record.children.length > 0) { return true } return false }, firstWidth(index) { if (index === 0) { return '150' } else { return '100' } }, firstAlign(index) { if (index === 0) { return 'left' } else { return 'center' } } } } 
</script> 
<style scoped>
.table-box {
    width: 100%;
